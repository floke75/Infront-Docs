---
title: "OrderEntryWidgetV2Options"
qualified_name: "WTK.OrderEntryWidgetV2Options"
kind: class
...
nav_path: "WTK > Widget Options > OrderEntryWidgetV2Options"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.OrderEntryWidgetV2Options.html"
defines: ["OrderEntryWidgetV2Options"]
member_count: 27
members: ["instrument?", "decimals", "hasContentCallback", "id", "linkChannels", "navButton", "navButtonClicked", "noContentPlaceholder?", "onChanges?", "persistState", "popupContainer?", "resetPopupPosition?", "showSearchWidget", "storageType", "streaming", "useOriginal", "usePerfectScrollbar", "widgetStateCallback", "widgetTitle", "autoLabels?", "confirmation?", "events?", "modify?", "onModifyHydration?", "portfolioId?", "sections?", "validity?"]
member_groups:
  ...
  "OrderEntryWidgetV2": ["autoLabels?", "confirmation?", "events?", "modify?", "onModifyHydration?", "portfolioId?", "sections?", "validity?"]
reference_count: 9
references: ["SDK.InfrontSDK.SymbolId", "WTK.WidgetDecimals", "WTK.WidgetNavButtonType", "WTK.WidgetPopupParent", "WTK.WidgetState", "WTK.OrderEntrySection", "WTK.OrderEntryWidgetV2", "WTK.SvelteWidgetOptions", "WTK.InfrontWidget"]
---

# OrderEntryWidgetV2Options

OrderEntryWidgetOptions class extending SvelteSingleInstrumentWidgetBaseOptions class.

Set Options for the OrderEntryWidget.

#### See

OrderEntryWidget | SvelteWidgetOptions | SvelteSingleInstrumentWidgetBaseOptions
...
Enable automatic label generation for components without explicit labels.

When enabled, components without a user-specified label will automatically receive
a human-readable label derived from their component type (e.g., "orderType" becomes "Order Type").
When disabled, components without explicit labels will have no label displayed.
...
Enable order confirmation dialog.

If enabled, the pre-defined action button "CONFIRM" must be available in configuration.

##### Remarks

not clear if this required!
...
```ts
events?: (name: EventName, data?: unknown) => void
```

A generic callback that notifies about different order-entry events.

##### Type Declaration

- (name: EventName, data?: unknown): void
...
```ts
modify?: { orderId: string; portfolioId: string }
```

If set, the widget will load this order for modification, both orderId and portfolioId are required to id the order.

##### Default Value

`undefined`

#### `Optional` onModifyHydration
...
Interceptor function to customize values when hydrating fields from a modified order.
This function is called for every core field being hydrated.

##### Type Declaration

- (key: string, value: unknown, props: Record<string, unknown>): unknown

  ##### Parameters

  - key: string

    The core field identifier (e.g. 'volume', 'price').
  - value: unknown

    The value calculated by the default coercion logic.
  - props: Record<string, unknown>

    The raw properties from the order being modified.

  ##### Returns unknown

  The final value to use for the component.
...
The default portfolio id to be selected.

This option is ignored if the widget is in modify mode.
...
```ts
sections?: OrderEntrySection[]
```

Order entry structure. If not set,
the widget will fall back to the default sections layout.
...
```ts
validity?: Validity = VALIDITY_OPTION_TODAY
```

The default validity option to be selected.
