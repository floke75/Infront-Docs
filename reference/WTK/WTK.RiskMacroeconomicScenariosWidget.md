---
title: "RiskMacroeconomicScenariosWidgetAlpha"
qualified_name: "WTK.RiskMacroeconomicScenariosWidget"
kind: class
...
module: "WTK"
group: "Risk Widgets"
nav_path: "WTK > Risk Widgets > RiskMacroeconomicScenariosWidget"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.RiskMacroeconomicScenariosWidget.html"
defines: ["RiskMacroeconomicScenariosWidget"]
member_count: 41
...
related_types: ["WTK.RiskMacroeconomicScenariosWidgetOptions"]
reference_count: 6
references: ["WTK.UI", "WTK.RiskMacroeconomicScenariosWidgetOptions", "WTK.WidgetState", "SDK.InfrontSDK.SymbolId", "WTK.WidgetBase", "WTK.InfrontWidget"]
---

# RiskMacroeconomicScenariosWidgetAlpha

RiskMacroeconomicScenariosWidget class extending SvelteSingleInstrumentWidgetBase class.

`InfrontWidget = infront.riskMacroeconomicScenariosWidget(target, [options]);`

The RiskMacroeconomicScenarios widget shows three different charts for macroeconomic scenarios.

1. Global market behavior

The global market behavior view shows the sensitivity of the portfolio value to a benchmark equity index (e.g. S&P 500).
Thereby the user can select from a set of scenarios (e.g. pessimistic).

1. Interest rate behaviour

The interest rate behavior widget shows the sensitivity of the portfolio value to a interest rate movements (short/long term).
Thereby the user can select from a set of scenarios (e.g. pessimistic).

1. Interest rate forward curve

The interest rate forward curve widget shows the simulated forward rate curve generated by the simulation.

Options can be configured with RiskMacroeconomicScenariosWidgetOptions.

#### Example

```ts
<script type="text/javascript">
    var opts = new Infront.RiskMacroeconomicScenariosWidgetOptions({
        portfolioSource: 'wtk',
        portfolioId: '123456',
    });

    widget = infront.riskMacroeconomicScenariosWidget("RiskMacroeconomicScenariosWidget", opts);
</script>
...
<div id="RiskMacroeconomicScenariosWidget"></div>
```

#### See

RiskMacroeconomicScenariosWidgetOptions | WidgetBase | SvelteSingleInstrumentWidgetBase | [InfrontWidget](./WTK.InfrontWidget.md)

#### Hierarchy

- SvelteSingleInstrumentWidgetBase<RiskMacroeconomicScenariosWidgetOptions>
...
```ts
new RiskMacroeconomicScenariosWidget(
    parentElement: HTMLElement,
    infrontUI: UI,
    options: Partial<RiskMacroeconomicScenariosWidgetOptions>,
): RiskMacroeconomicScenariosWidget
```
...
##### Returns RiskMacroeconomicScenariosWidget

Overrides SvelteSingleInstrumentWidgetBase<RiskMacroeconomicScenariosWidgetOptions>.constructor
...
```ts
optionsProxy: OptionsStore<RiskMacroeconomicScenariosWidgetOptions>
```
...
##### Returns RiskMacroeconomicScenariosWidgetOptions
...
    element: HTMLElement,
    infrontUI: UI,
    options: RiskMacroeconomicScenariosWidgetOptions,
...
- infrontUI: UI
- options: RiskMacroeconomicScenariosWidgetOptions
- widgetFn: SvelteWidgetFn<RiskMacroeconomicScenariosWidgetOptions, Record<string, any>>
...
```ts
validateOptions(options: RiskMacroeconomicScenariosWidgetOptions): void
```
