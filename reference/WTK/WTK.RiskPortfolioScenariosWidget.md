---
title: "RiskPortfolioScenariosWidgetAlpha"
qualified_name: "WTK.RiskPortfolioScenariosWidget"
kind: class
...
module: "WTK"
group: "Risk Widgets"
nav_path: "WTK > Risk Widgets > RiskPortfolioScenariosWidget"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.RiskPortfolioScenariosWidget.html"
defines: ["RiskPortfolioScenariosWidget"]
member_count: 41
...
related_types: ["WTK.RiskPortfolioScenariosWidgetOptions"]
reference_count: 6
references: ["WTK.UI", "WTK.RiskPortfolioScenariosWidgetOptions", "WTK.WidgetState", "SDK.InfrontSDK.SymbolId", "WTK.WidgetBase", "WTK.InfrontWidget"]
---

# RiskPortfolioScenariosWidgetAlpha

RiskPortfolioScenariosWidget class extending SvelteSingleInstrumentWidgetBase class.

`InfrontWidget = infront.riskPortfolioScenariosWidget(target, [options]);`

The portfolio scenarios widget shows a table with portfolio values depending on different scenarios (stress, pessimistic, moderate, optimistic).
This scenario distribution is additionally plotted on the right side in a bar chart.
As a reference a horizontal line with the current portfolio value is added to the bar chart.

Options can be configured with RiskPortfolioScenariosWidgetOptions.

#### Example

```ts
<script type="text/javascript">
    const opts = new Infront.RiskPortfolioScenariosWidgetOptions({
        portfolioSource: 'wtk',
        portfolioId: '123456',
    });

    widget = infront.riskPortfolioScenariosWidget("RiskPortfolioScenariosWidget", opts);
</script>
...
<div id="RiskPortfolioScenariosWidget"></div>
```

#### See

RiskPortfolioScenariosWidgetOptions | WidgetBase | SvelteSingleInstrumentWidgetBase | [InfrontWidget](./WTK.InfrontWidget.md)

#### Hierarchy

- SvelteSingleInstrumentWidgetBase<RiskPortfolioScenariosWidgetOptions>
...
```ts
new RiskPortfolioScenariosWidget(
    parentElement: HTMLElement,
    infrontUI: UI,
    options: Partial<RiskPortfolioScenariosWidgetOptions>,
): RiskPortfolioScenariosWidget
```
...
##### Returns RiskPortfolioScenariosWidget

Overrides SvelteSingleInstrumentWidgetBase<RiskPortfolioScenariosWidgetOptions>.constructor
...
```ts
optionsProxy: OptionsStore<RiskPortfolioScenariosWidgetOptions>
```
...
##### Returns RiskPortfolioScenariosWidgetOptions
...
    element: HTMLElement,
    infrontUI: UI,
    options: RiskPortfolioScenariosWidgetOptions,
...
- infrontUI: UI
- options: RiskPortfolioScenariosWidgetOptions
- widgetFn: SvelteWidgetFn<RiskPortfolioScenariosWidgetOptions, Record<string, any>>
...
```ts
validateOptions(options: RiskPortfolioScenariosWidgetOptions): void
```
