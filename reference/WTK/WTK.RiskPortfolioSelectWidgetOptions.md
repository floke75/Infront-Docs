---
title: "RiskPortfolioSelectWidgetOptions"
qualified_name: "WTK.RiskPortfolioSelectWidgetOptions"
kind: class
...
member_groups:
  ...
  "Other": ["constructor"]
  "RiskPortfolioSelectWidget": ["autoSelectFirstItem", "closeOnSelect", "defaultLabel?", "disabled", "loadingLabel?", "onPortfolioSelected?", "portfolioSource", "selectedPortfolioId", "showArrow", "showLoadingLabel", "showSelectedAsLabel"]
reference_count: 9
references: ["SDK.InfrontSDK.SymbolId", "WTK.WidgetDecimals", "WTK.WidgetNavButtonType", "WTK.WidgetState", "SDK.InfrontSDK.PortfolioRisk.RiskListPortfolio", "WTK.RiskPortfolioSelectWidget", "WTK.SvelteWidgetOptions", "WTK.InfrontWidget", "SDK.InfrontSDK.PortfolioRisk.RiskClientInfo"]
---

# RiskPortfolioSelectWidgetOptions

RiskPortfolioSelectWidgetOptions class extending SvelteSingleInstrumentWidgetBaseOptions class.

Set Options for the RiskPortfolioSelectWidget.

#### See

RiskPortfolioSelectWidget | SvelteWidgetOptions
...
```ts
new RiskPortfolioSelectWidgetOptions(
    initOptions?: Partial<RiskPortfolioSelectWidgetOptions>,
): RiskPortfolioSelectWidgetOptions
```

Initializes a new [RiskPortfolioSelectWidgetOptions](#).
You can optionally specify an init object that specifies
a subset of properties to set on the new options object.

##### Parameters

- `Optional` initOptions: Partial<RiskPortfolioSelectWidgetOptions>

  The initial property values to set.

##### Returns RiskPortfolioSelectWidgetOptions

Overrides SvelteSingleInstrumentWidgetBaseOptions.constructor

### RiskPortfolioSelectWidget
...
If set to `true` and no portfolio has been selected, the first portfolio in the dropdown list will be selected.
...
Can be toggled via a call to `RiskPortfolioSelectWidget`'s ` modify()` call.
...
You can turn this message off by setting [RiskPortfolioSelectWidgetOptions.showLoadingLabel](#showloadinglabel) to `false`.

Defaults to "(loading...)".

#### `Optional` onPortfolioSelected

```ts
onPortfolioSelected?: (portfolio: RiskListPortfolio) => void
```
...
Sets the portfolioId (as defined in the hosting application!) that should be selected by default.

Note: This id is called clientId or portfolioClientId in Risk Backend.

##### Default Value

`null` - null instead of undefined, as it is storable!
...
while the portfolio list is still loading, or to use the [RiskPortfolioSelectWidgetOptions.defaultLabel](#defaultlabel)
instead.
...
[RiskPortfolioSelectWidgetOptions.defaultLabel](#defaultlabel)
is shown instead.
