---
title: "RiskPortfolioScenariosWidgetOptions"
qualified_name: "WTK.RiskPortfolioScenariosWidgetOptions"
kind: class
...
nav_path: "WTK > Widget Options > RiskPortfolioScenariosWidgetOptions"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.RiskPortfolioScenariosWidgetOptions.html"
defines: ["RiskPortfolioScenariosWidgetOptions"]
member_count: 46
members: ["instrument?", "decimals", "hasContentCallback", "id", "linkChannels", "navButton", "navButtonClicked", "noContentPlaceholder?", "onChanges?", "persistState", "popupContainer?", "resetPopupPosition?", "showSearchWidget", "storageType", "streaming", "useOriginal", "usePerfectScrollbar", "widgetStateCallback", "widgetTitle", "chartHeight?", "chartWidth?", "colors", "columnOrder", "firstColumnFixed", "groupPadding", "lineColor", "pointPadding", "portfolioId", "portfolioSource", "positionId", "scenarioQuantiles", "showLegend", "showQuantiles", "showTitle", "showTooltip", "showValueAtRisk", "sortBy", "sortDirection", "spacingBottom", "spacingLeft", "spacingRight", "spacingTop", "tickPixelInterval", "valuationDatetime", "valueMode", "viewMode"]
member_groups:
  ...
  "RiskPortfolioScenariosWidget": ["chartHeight?", "chartWidth?", "colors", "columnOrder", "firstColumnFixed", "groupPadding", "lineColor", "pointPadding", "portfolioId", "portfolioSource", "positionId", "scenarioQuantiles", "showLegend", "showQuantiles", "showTitle", "showTooltip", "showValueAtRisk", "sortBy", "sortDirection", "spacingBottom", "spacingLeft", "spacingRight", "spacingTop", "tickPixelInterval", "valuationDatetime", "valueMode", "viewMode"]
reference_count: 8
references: ["SDK.InfrontSDK.SymbolId", "WTK.WidgetDecimals", "WTK.WidgetNavButtonType", "WTK.WidgetPopupParent", "WTK.WidgetState", "WTK.RiskPortfolioScenariosWidget", "WTK.SvelteWidgetOptions", "WTK.InfrontWidget"]
---

# RiskPortfolioScenariosWidgetOptions

RiskPortfolioScenariosWidgetOptions class extending SvelteSingleInstrumentWidgetBaseOptions class.

Set Options for the RiskPortfolioScenariosWidget.

#### See

RiskPortfolioScenariosWidget | SvelteWidgetOptions | SvelteSingleInstrumentWidgetBaseOptions
...
Define colors for positive and negative value bars.

First color is for positive values, second color is for negative values.

Or set colors by css-variables:

- --wtk-risk-portfolio-scenarios-options-bar-color
- --wtk-risk-portfolio-scenarios-options-bar-negative-color

The color for positive and negative values will be the same, if no second / negative color is set.

##### Default Value

`['#EB003F', '#E38FA6']` - default set by failover

#### columnOrder

```ts
columnOrder: ScenarioColumnField[] = ...
```

Order of the table columns.

Columns not listed are hidden. The name column is always the label anchor and may be placed anywhere.

##### Default Value

```ts
['name', 'current', 'stress', 'pessimistic', 'moderate', 'optimistic']
```

#### firstColumnFixed

```ts
firstColumnFixed: boolean = true
```

Lock the name column to the left, preventing it from being moved or reordered over.
...
Together with [RiskPortfolioScenariosWidgetOptions.pointPadding](#pointpadding) this option can be used to optimize the column (bar) sizes.
...
Define color of the line representing the current portfolio or positions value.

Or set color by css-variable: --wtk-risk-portfolio-scenarios-options-current-line-color

##### Default Value

`'#5D85AE'` - default set by failover

#### pointPadding

```ts
pointPadding: number = 0.05
```

Padding between each column or bar, in x axis units.

Together with [RiskPortfolioScenariosWidgetOptions.groupPadding](#grouppadding) this option can be used to optimize the column (bar) sizes.
...
```ts
showQuantiles: boolean = true
```

Show or hide the Quantiles values in table and chart.
...
```ts
showValueAtRisk: boolean = true
```

Show or hide the value-at-risk information.
...
```ts
sortBy: ScenarioColumnField = 'current'
```
...
Sort direction for the column specified in [sortBy](#sortby).
...
```ts
tickPixelInterval: number = 60
```

Minimum pixel distance between Y-axis tick marks. Larger values produce fewer ticks.
...
```ts
valueMode: "absolute" | "percent" = 'absolute'
```

Display scenario values as percentage change from the current value instead of absolute values.

When set to 'percent', the chart bars and the scenario columns in the table (stress, pessimistic,
moderate, optimistic) show the percentage change relative to the current value.
The current column always shows the absolute value as a reference anchor.
...
Which part of the widget to render.

Place two instances on the same linkChannels with `viewMode: 'table'` and ` viewMode: 'chart'`
to show both views side by side while sharing a single SDK data request via the cache.
