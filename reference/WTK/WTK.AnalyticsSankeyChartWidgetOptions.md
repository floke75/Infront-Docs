---
title: "AnalyticsSankeyChartWidgetOptions"
qualified_name: "WTK.AnalyticsSankeyChartWidgetOptions"
kind: class
...
nav_path: "WTK > Widget Options > AnalyticsSankeyChartWidgetOptions"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.AnalyticsSankeyChartWidgetOptions.html"
defines: ["AnalyticsSankeyChartWidgetOptions"]
member_count: 29
members: ["instrument?", "decimals", "hasContentCallback", "id", "linkChannels", "navButton", "navButtonClicked", "noContentPlaceholder?", "onChanges?", "persistState", "popupContainer?", "resetPopupPosition?", "showSearchWidget", "storageType", "streaming", "useOriginal", "usePerfectScrollbar", "widgetStateCallback", "widgetTitle", "expensesNodeColor", "fundamentalPeriodCount", "fundamentalSource", "onSelectedYearChanged?", "profitNodeColor", "selectedYear", "showNodeTooltip", "showPathTooltip", "showYearSelectDropdown", "startingNodeColor"]
member_groups:
  ...
  "AnalyticsSankeyChartWidget": ["expensesNodeColor", "fundamentalPeriodCount", "fundamentalSource", "onSelectedYearChanged?", "profitNodeColor", "selectedYear", "showNodeTooltip", "showPathTooltip", "showYearSelectDropdown", "startingNodeColor"]
reference_count: 9
references: ["SDK.InfrontSDK.SymbolId", "WTK.WidgetDecimals", "WTK.WidgetNavButtonType", "WTK.WidgetPopupParent", "WTK.WidgetState", "SDK.InfrontSDK.CoreDataAPI.CdapiEquityCompanyFundamentalSource", "WTK.AnalyticsSankeyChartWidget", "WTK.SvelteWidgetOptions", "WTK.InfrontWidget"]
---

# AnalyticsSankeyChartWidgetOptions

AnalyticsSankeyChartWidgetOptions class extending SvelteSingleInstrumentWidgetBaseOptions class.

Set Options for the AnalyticsSankeyChartWidget.

#### See

AnalyticsSankeyChartWidget | SvelteWidgetOptions | SvelteSingleInstrumentWidgetBaseOptions
...
```ts
expensesNodeColor: string = DEFAULT_EXPENSES_NODE_COLOR
```

Color of expenses nodes

Works only with valid color in Hexadecimal format
...
```ts
fundamentalPeriodCount: number = 5
```

How many periods of Fundamentals to show in dropdown
...
#### `Optional` onSelectedYearChanged

```ts
onSelectedYearChanged?: (selectedYear: number) => void
```

Callback fired when the user selects a year in the dropdown.

#### profitNodeColor

```ts
profitNodeColor: string = DEFAULT_PROFIT_NODE_COLOR
```

Color of profit nodes
...
The year data to diplay the chart for
When undefined, latest available year data is diplayed in the chart
...
```ts
showNodeTooltip: boolean = true
```

Display Node Tooltip in chart
...
```ts
showPathTooltip: boolean = true
```

Display Path Tooltip in chart

##### Default Value

`false`

#### showYearSelectDropdown

```ts
showYearSelectDropdown: boolean = false
```

Display Year Select Dropdown
...
```ts
startingNodeColor: string = DEFAULT_STARTING_NODE_COLOR
```

Color of starting node
