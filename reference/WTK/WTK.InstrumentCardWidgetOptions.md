---
title: "InstrumentCardWidgetOptions"
qualified_name: "WTK.InstrumentCardWidgetOptions"
kind: class
...
nav_path: "WTK > Widget Options > InstrumentCardWidgetOptions"
source_url: "https://docs.infrontfinance.com/docs/classes/WTK.InstrumentCardWidgetOptions.html"
defines: ["InstrumentCardWidgetOptions"]
member_count: 22
members: ["instrument?", "decimals", "hasContentCallback", "id", "linkChannels", "navButton", "navButtonClicked", "noContentPlaceholder?", "onChanges?", "persistState", "popupContainer?", "resetPopupPosition?", "showSearchWidget", "storageType", "streaming", "useOriginal", "usePerfectScrollbar", "widgetStateCallback", "widgetTitle", "resetLastBeforeTrade", "showExchange", "subscribe"]
member_groups:
  ...
  "InstrumentCardWidget": ["resetLastBeforeTrade", "showExchange", "subscribe"]
reference_count: 7
references: ["SDK.InfrontSDK.SymbolId", "WTK.WidgetDecimals", "WTK.WidgetNavButtonType", "WTK.WidgetPopupParent", "WTK.WidgetState", "WTK.InstrumentCardWidget", "WTK.InfrontWidget"]
---

# InstrumentCardWidgetOptions

InstrumentCardWidgetOptions class extending SvelteSingleInstrumentWidgetBaseOptions.

#### See

InstrumentCardWidget
...
#### resetLastBeforeTrade

```ts
resetLastBeforeTrade: boolean = false
```

When `false` (default), price and percent use ` PreLastTradedAt` / ` PreLastChangePct`,
which fall back to the previous trading day and never show zero before the first trade.
When `true`, uses ` Last` / ` PreChangeIntradayPercent`, which reset to zero before the first trade.
...
When `true`, shows the feed exchange short name between the ticker and currency.
