...
This property will be used only when translate property has been used. This allow to specify which value should be used to sort the column. If this property is true then the column will be sorted based on the original value before translation.
...
Allows to print a zero-value if datatype is nummeric and its value is zero.
...
This property will activate blink-animation used to indicate a value has changed. if it is assigned, the field will be given the class cell-update-up or cell-update-down if blinkType is UpDown or the class cell-update-neutral if blinkType is Infront.BlinkType.Neutral. Valid values are:

- Infront.BlinkType.Neutral
- Infront.BlinkType.UpDown
- Infront.BlinkType.None
...
If it is assigned, the column will not be moveable when [enableColumnsMove](https://doc.infrontfinance.com/v3/CoreObjects#enableColumnsMove) has been used. This property can be "left", "right" and "middle".
...
Use this property to prevent the user from deleting the column.
